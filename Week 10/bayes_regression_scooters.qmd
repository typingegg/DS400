...
bikes_scooters_1 <- bikes_scooters %>% 
  filter(!is.na(scooter_rides))

view(bikes_scooters_1)
```

```{r}
ggplot(bikes_scooters_1, aes(rides, scooter_rides)) +
  geom_point()

ggplot(bikes_scooters_1, aes(season, scooter_rides)) +
  geom_col()

avg_scooter_rides <-bikes_scooters %>% 
  group_by(day_of_week) %>% 
  summarise(mean_rides = mean(scooter_rides, na.rm = TRUE))

ggplot(avg_scooter_rides, aes(day_of_week, mean_rides)) +
