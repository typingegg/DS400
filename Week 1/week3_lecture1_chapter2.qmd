...
Build a prior probability model (an unconditional probability model) with probabilities for where the person is from
...
```{r}
prior_pop <- sum(prior_region*likelihood)
prior_pop
```
...
3\) Likelihood. Build a posterior probability model determining the probability that the person is from the south based on using the word "pop": P(S\|A)\
...
```{r}
prob_calc = ((prior_region[3]*likelihood[3])/prior_pop)
prob_calc
```
